...
from vector_prediction_model.dataloader import (
    Pose2ContactStateDataset,
    Wrench2ContactStateDataset,
)
...
class BaseContactStateTrainer:
    ...
    def __init__(self, training_folder, training_param_dict):
        ...
        # Initialize wandb
        wandb.init(
            project=training_param_dict.get(
                "wandb_project", "contact-state-prediction"
            ),
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if epoch % self.log_freq == 0:
                ...
                logger.debug(
                    f"Avg time per iter: {(datetime.datetime.now() - start_time).total_seconds() / (n + 1):.4f}s"
                )
                logger.debug("\n")
