...
class BaseContactStateDataset(Dataset):
    ...
    def __init__(
        self, data_path: str, filter_out_no_contact: bool = True, seed: int = 42
    ):
        """
        ...
        self.peg_hole_classes = [
            f"{h}-{p}" for h in self.hole_classes for p in self.peg_classes
        ]
...
class Wrench2ContactStateDataset(BaseContactStateDataset):
    ...
    def _process_data(self) -> None:
        ...
        self.peg_hole_classes_nonzero = [
            cls for cls in self.peg_hole_classes if self.data[cls].sum() > 0
        ]

        # Select relevant columns
        input_columns = (
            self.wrench_column_headings + self.normalized_wrench_column_headings
        )
